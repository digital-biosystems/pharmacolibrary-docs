...
## Model component
<dbs-pgx drug="salbutamol" model-id="Salbutamol_Vet2020_ipv" status="rejected" stale="false" population="children with refractory status asthmaticus" measured-compound="R-salbutamol, S-salbutamol" parameterization="mechanistic" topology="general_linear"></dbs-pgx>
...
|---|---|---|---|---|---|---|---|---|---|---|
| Vc | `Q63` · V1 | —(suppressed) | L | — | L | not captured | exact (1.0) | Vet_2020:other_prose | — | not captured |
| CL (L h−1) | `Q22` · CL | —(suppressed) | L h−1 | — | L/h | not captured | review_gapfill (0.7) | Courlet_2022:review | — | not captured |
...
- dropped value-less row: 'Baseline dose (μg)' (captured trailing unit 'μg' for child rows)
- dropped value-less row: 'CLR-salbutamol (L/h/70 kg)' (captured trailing unit 'L/h/70 kg' for child rows)
- dropped value-less row: 'CLS-salbutamol (L/h/70 kg)' (captured trailing unit 'L/h/70 kg' for child rows)
- dropped value-less row: 'Vc (L/70 kg)' (captured trailing unit 'L/70 kg' for child rows)
- salvaged Q63 ('Vc'=2.76) from results prose — parameter table was unreadable
...
- population split: 'ipv (%)' subgroup of Vet_2020 (paper reports 3 populations: estimation, ipv (%), shrinkage (%))
...
| C9_phys_window_Q63 | pass | volume within physiological range | 2.76 L | not captured | not captured | ['Vet_2020:other_prose'] |
...
- scholar stages: `../../../knowledgebase/drugs/drug_salbutamol/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Vet_2020` / `Vet_2020::ipv`)
