...
## Model component
<dbs-pgx drug="doxycycline" model-id="Doxycycline_Mileva2021_units" status="model_quarantined" stale="false" population="mature and immature rabbits" measured-compound="doxycycline" parameterization="mechanistic" topology="1C"></dbs-pgx>
...
|---|---|---|---|---|---|---|---|---|---|---|
| tvV | `Q61` · V | —(suppressed) | not captured | — | not captured | not captured | tv_prefix (0.95) | antibiotics-10-00310-t001:row2:col3 | — | not captured |
| tVCl | `Q22` · CL | —(suppressed) | not captured | — | not captured | not captured | tv_prefix (0.95) | antibiotics-10-00310-t001:row3:col3 | — | not captured |
| Half-life k10 | `Q57` · t1/2z | —(suppressed) | not captured | — | not captured | not captured | llm_corrected (0.6) | antibiotics-10-00310-t001:row6:col3 | — | not captured |
| AUC0-∞ | `Q17` · AUC∞ | —(suppressed) | not captured | — | not captured | not captured | exact (1.0) | antibiotics-10-00310-t001:row7:col3 | — | not captured |
...
- dropped unlinked row (NIL): 'Thetas (typical value)' — extend the ontology if this is a real PK parameter (source ['antibiotics-10-00310-t001:row1:col3'])
- routed 'stdev0' → Q315 (sigma) to residual_error — variability estimate, not a structural parameter
- dropped unlinked row (NIL): 'Secondary parameters' — extend the ontology if this is a real PK parameter (source ['antibiotics-10-00310-t001:row5:col3'])
- apparent-ness (ontology-grounded): parameterization=mechanistic, measured_compound=doxycycline
- population split: 'units' subgroup of Mileva_2021 (paper reports 3 populations: estimates, parameter name, units)
...
| C6_cl_magnitude | pass | &lt;= 90.0 L/h | 0.08 | not captured | not captured | ['antibiotics-10-00310-t001:row3:col3'] |
...
- scholar stages: `../../../knowledgebase/drugs/drug_doxycycline/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Mileva_2021` / `Mileva_2021::units`)
- model: `../../../knowledgebase/drugs/drug_doxycycline/models/modelica/_needs_review/Doxycycline_Mileva2021_units.mo`
- deviation: `../../../knowledgebase/drugs/drug_doxycycline/models/modelica/_needs_review/Doxycycline_Mileva2021_units.deviation.json`
