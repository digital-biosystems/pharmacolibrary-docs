...
## Citation
Wright DF; Stamp LK; Merriman TR; Barclay ML; Duffull SB; Holford NH et al. (2013). European journal of clinical pharmacology 69
  ·  DOI: [10.1007/s00228-013-1478-8](https://doi.org/10.1007/s00228-013-1478-8)

## Model component
<dbs-pgx drug="allopurinol" model-id="Allopurinol_Wright2013_base" status="rejected" stale="false" population="patients with gout and healthy volunteers" measured-compound="oxypurinol" parameterization="mechanistic" topology="parent_metabolite"></dbs-pgx>
...
|---|---|---|---|---|---|---|---|---|---|---|
| peripheral volume; Q, inter- | `Q64` · V2 | 28.9 | not captured | not captured | not captured | not captured | llm_confirmed (0.6) | tab_1:row14:col2 | — | not captured |
| volume; F, bioavailability; ω, | `Q40` · Fab | 0.371 | not captured | not captured | not captured | not captured | llm_confirmed (0.6) | tab_1:row16:col2 | — | not captured |
...
- routed 'Corr(CL,V), correlation between' → Q314 (omega_cov) to covariance — variability estimate, not a structural parameter
- apparent-ness (ontology-grounded): parameterization=mechanistic, measured_compound=oxypurinol
- model-stage split: 'base sequential model' is the base model of Wright_2013 (paper reports 2 stages: base sequential model, final sequential model); same population, different model-building step
...
second reader `gpt-oss:120b` · first reading `qwen3.6:27b-q8_0` · agreement 0.75 (6/8 fields) · the first reading is what this page shows

<details><summary>2 field(s) the two readings disagree on</summary>

| field | first reading | second reading | agreement |
|---|---|---|---|
| `parameters[between-subject variance for oxypurinol clearance and volume of distribution]` | not captured | 50 | only_one_extracted |
| `parameters[decrease in between-subject variance for oxypurinol clearance and volume of distribution]` | 50 | not captured | only_one_extracted |
...
- scholar stages: `../../../knowledgebase/drugs/drug_allopurinol/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Wright_2013` / `Wright_2013::base`)
