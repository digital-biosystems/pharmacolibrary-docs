...
## Model component
<dbs-pgx drug="inositol" model-id="Inositol_Phelps2013_2_loglikelihood_p_value_e" status="rejected" stale="false" population="preterm infants" measured-compound="myo-inositol" parameterization="mechanistic" topology="1C"></dbs-pgx>
...
|---|---|---|---|---|---|---|---|---|---|---|
| central volume of distribution | `Q61` · V | —(suppressed) | l/kg | — | L | not captured | boundary (0.8) | Phelps_2013:discussion_prose | — | not captured |
...
- column '−2 loglikelihood(p-value e)' classified 'other' by the LLM but kept: the deterministic diagnostic-column test disagrees (a stratum column is a value column, not a statistic)
- dropped unlinked row (NIL): 'log(Size)' — extend the ontology if this is a real PK parameter (source ['T7:row5:col6'])
- dropped unlinked row (NIL): 'GA Strata' — extend the ontology if this is a real PK parameter (source ['T7:row8:col6'])
- dropped unlinked row (NIL): 'Creatinine Clearance' — extend the ontology if this is a real PK parameter (source ['T7:row11:col6'])
- dropped unlinked row (NIL): 'All Covariates' — extend the ontology if this is a real PK parameter (source ['T7:row14:col6'])
...
- population split: '−2 loglikelihood(p-value e)' subgroup of Phelps_2013 (paper reports 5 populations: covariate estimate(standard error), estimate, parametername a, standard error, −2 loglikelihood(p-value e))

**Extraction notes:**
- unparsed cell T7:row2:col6 = '2708.2 (NA)'
- companion parameter table 4 transcribed (15 record(s))
- LLM selected parameter table(s) 4, 7
...
- scholar stages: `../../../knowledgebase/drugs/drug_inositol/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Phelps_2013` / `Phelps_2013::2_loglikelihood_p_value_e`)
