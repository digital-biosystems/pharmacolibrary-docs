...
|---|---|---|---|---|---|---|---|---|---|---|
| KP,NUC | `Q350` · kic | 0.5 | Zhang et al., 2020 | not captured | [zhangetal] | not captured | exact (1.0) | T1:row5:col3 | — | not captured |
| theta_kic_im | `Q900` · theta_kic_im | 0.19 | not captured | not captured | not captured | not captured | not captured (not captured) | T1:row4:col3 | — | not captured |
| theta_kic_im | `Q900` · theta_kic_im | 0.19 | not captured | not captured | not captured | not captured | not captured (not captured) | T1:row8:col3 | — | not captured |
...
- dropped unlinked row (NIL): 'QRDV' — extend the ontology if this is a real PK parameter (source ['T1:row1:col3'])
- dropped unlinked row (NIL): 'QIM' — extend the ontology if this is a real PK parameter (source ['T1:row2:col3'])
- dropped unlinked row (NIL): 'QNUC' — extend the ontology if this is a real PK parameter (source ['T1:row3:col3'])
- unit_dimension_unknown: 'Zhang et al., 2020' (kic)
- dropped duplicate Q350 ('KP,NTP', value '8.08E10') — already have one for this compound
- dropped duplicate Q350 ('KC,NUC', value '0.85') — already have one for this compound
- dropped unlinked row (NIL): 'CLC,RDV' — extend the ontology if this is a real PK parameter (source ['T1:row9:col3'])
...
| `gpt-oss:120b` | `parameters[qrdv]` | not captured | 0.19 | only_one_extracted |
...
| C5_unit_missing_Q350 | fail | 1 / [time] | Zhang et al., 2020 | not captured | not captured | ['T1:row5:col3'] |
