...
# QTcF interval change from baseline — PD  <span class="pk-badge pk-badge--red">rejected</span> <span class="pk-badge pk-badge--red" title="re-read by gpt-oss:120b (not confirmed, agreement 0.667). The first reading is what the record holds.">cross-check: disputed</span>
...
- **driver:** `conc_no_pk`
- **tier:** population
- **effect:** unknown/additive

## Citation
van Dijkman SC; Félices M; Pandurangavittal B; Ghorpade S; Easterbrook C; Zabielski M; Della Pasqua O et al. (2026). British journal of clinical pharmacology 92
...
`Tamoxifen_van2026_PD_qtcf` — linear, `response = E0 + slope*exposure`

| parameter | value (paper units) | SI |
|---|---|---|
| E0 | 0.66 ms | 0.00066 s |
| slope | 0.01 ms/ng/mL | 10 meter ** 3 * second / kilogram |

Closed-form check points (response, SI): `at_0` = 0.00066, `per_exposure_unit` = 10

Deviations:

- `pd_binding_off_target_driver` — driver compound 'paroxetine' is not 'tamoxifen' nor one of its metabolites — the curve belongs to that compound's exposure (S12)
...
- off_target_driver: 'paroxetine' is not 'tamoxifen' (S12)
...
| `gpt-oss:120b` | `parameters[Q324]` | 0.66 | not captured | only_one_extracted |
