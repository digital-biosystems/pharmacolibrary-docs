...
**Second reading:** `gpt-oss:120b` read this paper differently on `model.bioavailability.theta` (44.3 vs not captured) and 3 more field(s) — a structural parameter, so the record is disputed.
...
**Extraction notes:**
- no GROBID TEI available — transcribed from abstract in Agoram_2006_metadata.yaml (3 record(s)); values are summary statistics, not a fitted model
...
|---|---|---|---|---|
| `gpt-oss:120b` | `model.bioavailability.theta` | 44.3 | not captured | only_one_extracted |
| `gpt-oss:120b` | `parameters[bioavailability]` | 44.3 | not captured | only_one_extracted |
| `gpt-oss:120b` | `parameters[clearance]` | 2010 | not captured | only_one_extracted |
| `gpt-oss:120b` | `parameters[steady-state volume of distribution]` | 3390 | not captured | only_one_extracted |
...
- scholar stages: `../../../knowledgebase/drugs/drug_darbepoetin_alfa/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Agoram_2006` / `Agoram_2006::reference`)
