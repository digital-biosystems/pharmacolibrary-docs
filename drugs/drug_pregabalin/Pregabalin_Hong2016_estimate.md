...
## Model component
<dbs-pgx drug="pregabalin" model-id="Pregabalin_Hong2016_estimate" status="model_quarantined" stale="false" population="healthy adults" measured-compound="pregabalin" parameterization="apparent" topology="1C"></dbs-pgx>
...
|---|---|---|---|---|---|---|---|---|---|---|
| θCLCR | `Q22` · CL | 0.511 | not captured | not captured | not captured | not captured | llm (0.6) | t5-dddt-10-3995:row4:col2 | — | not captured |
| V2/F | `Q82` · V2/F | 18.0 | not captured | not captured | not captured | not captured | exact (1.0) | t5-dddt-10-3995:row5:col2 | — | not captured |
| Q/F | `Q69` · Q/F | 26.5 | not captured | not captured | not captured | not captured | exact (1.0) | t5-dddt-10-3995:row6:col2 | — | not captured |
| V3/F | `Q78` · V3/F | 27.0 | not captured | not captured | not captured | not captured | exact (1.0) | t5-dddt-10-3995:row7:col2 | — | not captured |
| MTTfed | `Q81` · MTT | 0.879 | not captured | not captured | not captured | not captured | llm (0.6) | t5-dddt-10-3995:row9:col2 | — | not captured |
| NN | `Q311` · n_transit | 3.61 | not captured | not captured | not captured | not captured | exact (1.0) | t5-dddt-10-3995:row10:col2 | — | not captured |
| ωKa | `Q49` · kabs | 49.4 | not captured | not captured | not captured | not captured | llm_confirmed (0.6) | t5-dddt-10-3995:row21:col2 | — | not captured |
| CLt/F | `Q900` · CLt/F | 6.25 | not captured | not captured | not captured | not captured | not captured (not captured) | t5-dddt-10-3995:row3:col2 | — | not captured |
...
|---|---|---|---|
| CLt/F | Q27 | not captured | llm |

## Departures & gaps

**Interpretation flags:**
- dropped unlinked row (NIL): 'MTTfast' — extend the ontology if this is a real PK parameter (source ['t5-dddt-10-3995:row8:col2'])
- dropped unlinked row (NIL): 'Kafast' — extend the ontology if this is a real PK parameter (source ['t5-dddt-10-3995:row11:col2'])
- dropped unlinked row (NIL): 'Kafed' — extend the ontology if this is a real PK parameter (source ['t5-dddt-10-3995:row12:col2'])
- dropped duplicate Q27 ('ωCLt/F', value '11.2') — already have one for this compound
- dropped duplicate Q82 ('ωV2/F', value '27.8') — already have one for this compound
- dropped duplicate Q81 ('ωMTTfast', value '43.7') — already have one for this compound
- dropped duplicate Q81 ('ωMTTfed', value '81.9') — already have one for this compound
- dropped unlinked row (NIL): 'ρCLt/F−V2/F' — extend the ontology if this is a real PK parameter (source ['t5-dddt-10-3995:row22:col2'])
...
- Q27 (CL/F) is equation-defined: value moved to equation-variable 'CLt/F'; equation kept verbatim
- population split: 'estimate' subgroup of Hong_2016 (paper reports 2 populations: description (unit), estimate)
...
- scholar stages: `../../../knowledgebase/drugs/drug_pregabalin/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Hong_2016` / `Hong_2016::estimate`)
- model: `../../../knowledgebase/drugs/drug_pregabalin/models/modelica/_needs_review/Pregabalin_Hong2016_estimate.mo`
- deviation: `../../../knowledgebase/drugs/drug_pregabalin/models/modelica/_needs_review/Pregabalin_Hong2016_estimate.deviation.json`
