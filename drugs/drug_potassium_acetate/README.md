<div class="pk-crumbs" data-crumbs="[{&quot;label&quot;:&quot;Drugs&quot;,&quot;href&quot;:&quot;README.md&quot;},{&quot;label&quot;:&quot;B05X&quot;,&quot;href&quot;:&quot;atc/B05X.md&quot;},{&quot;label&quot;:&quot;potassium acetate&quot;}]"></div>

# potassium acetate

- **generic name:** potassium acetate
- **ATC codes:** `B05XA17`
- **DrugBank:** [DB14498](https://go.drugbank.com/drugs/DB14498)
- **groups:** approved

## About

**Indication.** Potassium is used to regulate hypokalemia as a primary condition or secondary to other medical conditions.

## Extraction summary

| extracted at | time | popPK e/r/o | PD e/r/o (paper) | PGx e/r/o | tokens in/out | LLM | papers | GROBID/JATS | OA/non-OA | NONMEM |
|---|---|---|---|---|---|---|---|---|---|---|
| 2026-09-08 05:42 | 1:27 | 0/0/0 | 0/1/0 | 0/0/0 | 11,184/483 | ollama / qwen3.8:27b-mtp-q8_0 | 15 | 0/15 | 15/0 | 0 |

## popPK records

_not available_

## Pharmacodynamics (PD)

| status | detail | citation | doi |
|---|---|---|---|
| <span class="pk-badge pk-badge--red">rejected</span> | [Rehan_2023](drugs/drug_potassium_acetate/pd_Rehan_2023_Arthritis_clinical_score.md) | Rehan S et al., Signal peptide mimicry primes Sec61 for…, Nature chemical biology (2023) | [10.1038/s41589-023-01326-1](https://doi.org/10.1038/s41589-023-01326-1) |
| <span class="pk-badge pk-badge--red">rejected</span> | [Rehan_2023](drugs/drug_potassium_acetate/pd_Rehan_2023_Cytokine_secretion.md) | Rehan S et al., Signal peptide mimicry primes Sec61 for…, Nature chemical biology (2023) | [10.1038/s41589-023-01326-1](https://doi.org/10.1038/s41589-023-01326-1) |
| <span class="pk-badge pk-badge--red">rejected</span> | [Rehan_2023](drugs/drug_potassium_acetate/pd_Rehan_2023_GLuc_activity.md) | Rehan S et al., Signal peptide mimicry primes Sec61 for…, Nature chemical biology (2023) | [10.1038/s41589-023-01326-1](https://doi.org/10.1038/s41589-023-01326-1) |

## Coverage

- **PubMed hits:** 25 matched, 24 returned
- **screened:** 0  ·  **relevant:** 0
- **records:** 0  ·  extracted 0  ·  needs_review 0  ·  rejected 0  ·  stale 0
- **scholar-agent fallback query used:** True

## Full text wanted

_1 paper(s) judged relevant from the abstract, with no full text on disk — paywalled, or the resolver could not reach them. Follow the DOI, then save the PDF into `papers/` as `&lt;save as&gt;.pdf` and re-run the extraction._

| save as | citation | domain | score | DOI | PubMed | why wanted |
|---|---|---|---|---|---|---|
| `Yu_1994.pdf` | Yu B et al., Interleukin-1 beta inhibits synaptic tr…, The Journal of pharmacology… (1994) | pd | 4 | not captured | [7525939](https://www.ncbi.nlm.nih.gov/pubmed/7525939) | metadata signals extractable PD data (EC50) |

<sub>queue written 2026-09-08T05:42:54.105919+00:00</sub>

## Screened and excluded

| domain | paper | verdict | relevance | extractability | reason |
|---|---|---|---|---|---|
| popPK | Arif_2026 | irrelevant | 0 | 0 | The paper is a study on rooster semen quality and selenium methionine, where potassium acetate is merely an ingredient in the semen extender, not the subject of pharmacokinetic analysis. |
| PD | Arif_2026 | not_relevant | 0 | 0 | The paper studies selenium methionine, not potassium acetate. |
| popPK | Garlid_1983 | irrelevant | 0 | 0 | The paper is an in-vitro mechanistic study on mitochondrial uncoupling by local anesthetics, not a pharmacokinetic study of potassium acetate. |
| PD | Garlid_1983 | not_relevant | 0 | 0 | The paper studies the mechanism of uncoupling by amine local anesthetics and does not report any pharmacodynamic or exposure-response data for potassium acetate. |
| popPK | Hauseman_2025 | irrelevant | 0 | 0 | The paper is a structural biology and medicinal chemistry study on RAS-mutant cancers and does not involve potassium acetate or pharmacokinetic parameters. |
| PD | Hauseman_2025 | not_relevant | 0 | 0 | The paper focuses on the discovery of SHOC2 inhibitors for RAS-mutant cancers and does not mention or analyze potassium acetate. |
| popPK | He_2004 | irrelevant | 0 | 0 | The paper is an agricultural chemistry study on phosphorus hydrolysis where potassium acetate is used only as a buffer, not as a subject drug for pharmacokinetic analysis. |
| popPK | Hoke_1988 | irrelevant | 0 | 0 | The study investigates the mechanism of mitochondrial dysfunction induced by a gold complex in isolated rat liver mitochondria, using potassium acetate only as a buffer, and does not report pharmacokinetic parameters for potassium acetate. |
| PD | Hoke_1988 | not_relevant | 0 | 0 | The paper studies the mechanism of action of a gold complex (Au(DPPE)+2) on mitochondria, not the pharmacodynamics of potassium acetate, which is only mentioned as a buffer component. |
| popPK | Hossain_2025 | irrelevant | 0 | 0 | The paper is a study on the synthesis and biological activity of silver nanoparticles, and potassium acetate is only mentioned as a reagent in the flavonoid assay, not as a subject drug for pharmacokinetic analysis. |
| PD | Hossain_2025 | not_relevant | 0 | 0 | The paper studies silver nanoparticles synthesized from plant extract, not potassium acetate, and does not report any pharmacodynamic or exposure-response relationship for the target drug. |
| popPK | Hua_1993 | irrelevant | 0 | 0 | The paper describes a ribonuclease mechanism in cell lines and mentions potassium acetate only as a buffer component, not as a subject drug for pharmacokinetic analysis. |
| popPK | Imani_2025 | irrelevant | 0 | 0 | The paper is a materials science study on wound dressings using lawsone, and potassium acetate is only listed as a reagent for buffer preparation, not as a subject drug for PK analysis. |
| PD | Imani_2025 | not_relevant | 0 | 0 | The paper focuses on the extraction of lawsone and the physicochemical characterization of a wound dressing; potassium acetate is listed only as a reagent, and no pharmacodynamic or exposure-response analysis is performed. |
| popPK | Jacobs_2025 | irrelevant | 0 | 0 | The paper is a catalysis study on vinyl acetate synthesis using potassium acetate as a promoter, not a pharmacokinetic study of the drug potassium acetate. |
| PD | Jacobs_2025 | not_relevant | 0 | 0 | The paper investigates heterogeneous catalysis and surface chemistry of palladium acetate species, not pharmacodynamics or drug exposure-response relationships. |
| popPK | Jiang_2015 | irrelevant | 0 | 0 | The paper is a microbiology study on bacterial bioreporters where potassium acetate is used as a carbon source, not a pharmacokinetic study of the drug. |
| popPK | Lee_2019 | irrelevant | 0 | 0 | The paper studies the pharmacokinetics of engineered IgG antibodies (Trastuzumab, Rituximab, etc.), not potassium acetate. |
| PD | Lee_2019 | not_relevant | 0 | 0 | The paper focuses on the pharmacokinetics (PK) of an engineered antibody Fc variant and its binding affinity to FcRn, but does not report a pharmacodynamic (PD) or exposure-response relationship for potassium_acetate. |
| popPK | Liang_2026 | irrelevant | 0 | 0 | The paper describes the synthesis and in vitro kinase inhibition of BMX inhibitors, not the pharmacokinetics of potassium acetate. |
| PD | Liang_2026 | not_relevant | 0 | 0 | The paper focuses on the medicinal chemistry and molecular modeling of covalent BMX kinase inhibitors, reporting in vitro potency (IC50/EC50) and binding kinetics, but contains no pharmacokinetic data or exposure-response (PD) analysis for potassium acetate or any other drug. |
| popPK | Lin_2025 | irrelevant | 0 | 0 | The paper is a radiochemistry study on Pb-203 production where potassium acetate is used only as a chemical eluent, not as a subject drug for pharmacokinetic analysis. |
| popPK | Naji_2025 | irrelevant | 0 | 0 | The paper is a phytochemical and pharmacological study of a plant extract (Corallocarpus glomeruliflorus) and does not report pharmacokinetic parameters for potassium acetate. |
| PD | Naji_2025 | not_relevant | 0 | 0 | The paper investigates the phytochemical and pharmacological properties of a plant extract (Corallocarpus glomeruliflorus) and does not mention or analyze potassium acetate. |
| popPK | Pozo_2026 | irrelevant | 0 | 0 | The paper is an in-vitro metabolomics study on glycine and hepatocyte maturation, containing no pharmacokinetic data for potassium acetate. |
| PD | Pozo_2026 | not_relevant | 0 | 0 | The paper investigates the metabolic effects of glycine on hepatocyte maturation and does not contain any data, analysis, or mention of potassium acetate. |
| popPK | Raig_2025 | irrelevant | 0 | 0 | The paper describes the development of LRRK2 kinase inhibitors for Parkinson's disease and does not involve potassium_acetate or report any pharmacokinetic parameters for it. |
| PD | Raig_2025 | not_relevant | 0 | 0 | The paper reports in vitro IC50 values and dose-response curves for LRRK2 inhibitors, but does not contain any data, analysis, or mention of potassium_acetate. |
| popPK | Rehan_2023 | irrelevant | 0 | 0 | The paper describes the structural biology and mechanism of action of KZR-8445 (a Sec61 inhibitor) and does not involve potassium acetate or report pharmacokinetic parameters for it. |
| popPK | Tulloch_2023 | irrelevant | 0 | 0 | The paper describes a drug resistance screening method for Leishmania parasites and does not involve potassium acetate or pharmacokinetic studies. |
| PD | Tulloch_2023 | not_relevant | 0 | 0 | The paper describes a barcoding method for assessing drug resistance in Leishmania and does not report any pharmacodynamic or exposure-response analysis for potassium acetate. |
| popPK | Yaeger_2021 | irrelevant | 0 | 0 | The study investigates the pharmacokinetics of tulathromycin, not potassium acetate (which is only mentioned as a buffer component in the analytical method). |
| PD | Yaeger_2021 | not_relevant | 0 | 0 | The paper reports pharmacokinetics of tulathromycin, not potassium_acetate, and contains no pharmacodynamic or exposure-response analysis. |
| popPK | Yu_1994 | irrelevant | 0 | 0 | The paper is a neurophysiology study where potassium acetate is used only as a component of a microelectrode, not as a subject drug for pharmacokinetic analysis. |
| PD | Yu_1994 | not_relevant | 0 | 0 | The paper reports a dose-response relationship for Interleukin-1 beta, not potassium acetate; potassium acetate is only used as an electrode filling solution to determine reversal potentials. |
| popPK | Zahiruddin_2021 | irrelevant | 0 | 0 | The paper is a study on the immunomodulatory activity of a polyherbal combination and does not report pharmacokinetic parameters for potassium acetate, which is only mentioned as a reagent in the flavonoid assay. |
| PD | Zahiruddin_2021 | not_relevant | 0 | 0 | The paper studies a polyherbal combination, not potassium acetate, and reports no pharmacodynamic parameters for the target drug. |
| popPK | unknown_2017 | irrelevant | 0 | 0 | The provided evidence contains only a conference acronym and no pharmacokinetic data or text regarding potassium acetate. |
| PD | unknown_2017 | not_relevant | 0 | 0 | The provided text is a conference tag (EANM'17) and contains no information regarding potassium acetate, pharmacodynamics, or exposure-response relationships. |

---
<sub>Generated by `docs.py` (scholarv2) from `knowledgebase/drugs/drug_potassium_acetate`. Every value traces to an artifact field; `not captured` = absent from the KB.</sub>
