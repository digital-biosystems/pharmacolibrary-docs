...
## Model component
<dbs-pgx drug="capecitabine" model-id="Capecitabine_Doshi2015_estimate_value" status="rejected" stale="false" population="patients with advanced or metastatic gastric/GEJ cancer" measured-compound="rilotumumab" parameterization="mechanistic" topology="1C"></dbs-pgx>
...
- dropped unlinked row (NIL): 'l (per day) a' — extend the ontology if this is a real PK parameter (source ['Doshi_2015_table_2:row1:col1'])
- dropped unlinked row (NIL): 'l (per day) b' — extend the ontology if this is a real PK parameter (source ['Doshi_2015_table_2:row2:col1'])
- dropped unlinked row (NIL): 'MET effect on l' — extend the ontology if this is a real PK parameter (source ['Doshi_2015_table_2:row3:col1'])
- dropped PD-category row 'r (per day)' → Q342 (lambda_hazard, category G14) — pharmacodynamic parameters belong to scholarpd, not the PK model (source ['Doshi_2015_table_2:row4:col1'])
- dropped PD-category row 'EC 50Cminss (mg/mL) a' → Q321 (EC50, category G11) — pharmacodynamic parameters belong to scholarpd, not the PK model (source ['Doshi_2015_table_2:row8:col1'])
- dropped PD-category row 'EC 50Cminss (mg/mL) c' → Q321 (EC50, category G11) — pharmacodynamic parameters belong to scholarpd, not the PK model (source ['Doshi_2015_table_2:row9:col1'])
- dropped PD-category row 'Hill' → Q325 (Hill, category G11) — pharmacodynamic parameters belong to scholarpd, not the PK model (source ['Doshi_2015_table_2:row11:col1'])
- apparent-ness (ontology-grounded): parameterization=mechanistic, measured_compound=rilotumumab
- population split: 'estimate value' subgroup of Doshi_2015 (paper reports 2 populations: estimate value, typical value)
...
- scholar stages: `../../../knowledgebase/drugs/drug_capecitabine/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Doshi_2015` / `Doshi_2015::estimate_value`)
