...
> ⚙️ **Pipeline limitation — scholar.** a reported unit is missing from the conversion table, so the parameter reached the engineer without an SI value<br><sub>evidence: `V/F`, `CL/F`, `CL`, `kabs`</sub>
...
## Model component
<dbs-pgx drug="ofloxacin" model-id="Ofloxacin_Fillastre1987_reference" status="not_modelled" stale="false" population="subjects with renal failure and healthy controls" measured-compound="ofloxacin" parameterization="apparent" topology="1C"></dbs-pgx>
...
- scholar stages: `../../../knowledgebase/drugs/drug_ofloxacin/papers/_screenv2.yaml`, `_locatev2.yaml`, `_transcribev2.yaml`, `_interpretv2.yaml`, `_validatev2.yaml`, `_reviewv2.yaml` (keys `Fillastre_1987` / `Fillastre_1987::subjects with renal failure and healthy controls`)
